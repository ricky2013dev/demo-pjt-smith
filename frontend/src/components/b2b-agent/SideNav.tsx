...
import { MAIN_MENU, FOOTER_MENU, PATIENT_LIST_PATH, NavItem, NavSubItem } from '@/constants/navigation';
...
const SideNav: React.FC<SideNavProps> = ({
  ...
  const renderSubMenu = (subMenu: NavSubItem[]) => (
      ...
      {subMenu
        ...
        .map((subItem) => (
          <button
            ...
            aria-current={activeTab === subItem.tab ? 'page' : undefined}
          ...
          >
            {/* Underline marks the open tab, so the row keeps its left alignment */}
            <span
  ...
  return (
    <aside className="hidden lg:flex w-60 shrink-0 flex-col border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-y-auto">
      <nav aria-label="Main navigation" className="flex flex-col gap-0.5 p-3">
        {MAIN_MENU.map((item) => {
          // Sub-menu only expands on its own page, and only where tabs are wired up
          const isExpanded = !!item.subMenu && path === item.path && !!onTabChange;

          // Expanded, the item and its tabs read as one section rather than two lists
          return isExpanded ? (
            <div key={item.path} className="my-1 pb-1">
              {renderItem(item)}
        ...
        {FOOTER_MENU.map((item) => (
          <React.Fragment key={item.path}>
            {/* Its own feature, so it sits well clear of the patient navigation */}
            {renderItem(item, 'mt-8')}
          </React.Fragment>
        ))}
      </nav>
    </aside>
